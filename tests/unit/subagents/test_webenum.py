"""Tests for the webenum sub-agent (Phase 1, Task 17).

Verifies that ``webenum_subagent`` chains ``httpx_probe`` →
``feroxbuster_dir`` + ``nuclei_scan`` (in parallel) and returns a
``WebEnumOutput`` containing all three results.
"""
import pytest
