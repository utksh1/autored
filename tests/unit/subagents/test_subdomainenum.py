"""Tests for the subdomainenum sub-agent (Phase 1, Task 18).

Verifies that ``subdomainenum_subagent`` runs ``subfinder_enum`` and
``amass_enum`` in parallel, then merges + dedupes the results into a
single ``SubdomainList``.
"""
import pytest
