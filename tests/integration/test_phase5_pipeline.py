"""Phase 5, Task 14 — Integration test: full Phase 5 pipeline, mocked.

End-to-end test that mocks every LLM + subprocess + subagent + EventBus
boundary in the Phase 5 graph, then runs ``build_phase5_graph`` and
asserts the final state is populated with the Phase 5 fields the
Lateral + Cleanup Agents own.

Layered on top of the Phase 4 T14 pattern by adding the Phase 5
sub-agent / spawner layer:

* ``pivotexecutor_subagent`` — returns a successful wmiexec PivotRecord
  targeting ``192.168.56.11`` (success=True, needs_tunnel=False so the
  TunnelSetup sub-agent is never invoked).
* ``tunnelsetup_subagent`` — defensive no-op (patched even though the
  pivot doesn't need a tunnel — mirrors the Phase 4 ``_maybe_run_blood-
  hound`` defensive patch pattern).
...
* ``artifactremover_subagent`` / ``verificationscanner_subagent`` —
  return typed CleanupResult payloads. Both ``verified=True`` because
  the pipeline-level assertion demands every cleanup result verified
  (the sub-agent isolation tests in Tasks 8/9 cover the verified=False
  failure path).
* ``cleanup.run_subprocess`` — defensive no-op (only called on
  tunnel teardowns; the pivot doesn't need a tunnel so this patch
  never fires).

State pre-seeding (LangGraph merges reducer fields): two GoAD hosts
(``192.168.56.22`` footholded, ``192.168.56.11`` un-footholded), one
Windows foothold (``f-1`` via ``psexec_smb`` winrm), one administrator
hash secret (``s-1`` sourced from ``secretsdump:SAM/administrator`` so
the Lateral Agent's username parser picks ``administrator`` and the
wmiexec pass-the-hash path is selected), one ``scheduled_task``
persistence artifact (``a-1`` with a ``schtasks`` removal_command
matching the cleanup sub-agent mock payloads), and one staged
``linpeas_upload.sh`` temp file in ``evidence_paths`` (so the Cleanup
Agent's ``_identify_temp_files`` substring match picks it up and the
unlink branch runs end-to-end).

The Phase 1-3 LLM / recon-tool / NVD / Chroma / DeepSeek / exploit
sub-agent / ``_verify_foothold`` mock lattice is copied verbatim from
``tests/integration/test_phase4_pipeline.py`` — the Phase 4 post-ex
...
verbatim. The Phase 5 layer is appended on top inside the same
``contextlib.ExitStack`` (~30+ patches — past Python's static ``with``
nesting limit).

Verifies the full Phase 5 chain runs as a single unit::
...
* Phase 1-4 carry-overs: phase == "done", ≥1 host / service / foothold,
  ≥1 local_users / harvested_secrets / persistence_artifacts.
* Phase 5 specific: ≥1 pivot with target_host=192.168.56.11 success=True;
  ≥1 movement_path from_host=192.168.56.22 to_host=192.168.56.11;
  exactly 1 sub_engagement with sub_id=p5-pipeline_sub_01 status=completed;
  ≥2 cleanup_results all verified.
...
- ``checkpointer=None`` keeps the test off the persistence layer
  ...
  even though the bus no longer rides on state — defensive
  belt-and-suspenders rather than the I1 workaround it used to be).
- ``hitl_mode = "always_ask"`` exercises the real HitL path through
  the I1 RunnableConfig channel — same choice as Phase 3 / 4 T14. The
  bus patches override the gate's wait → approve so the run completes
  deterministically.
- State pre-seeding is the brief's documented approach (LangGraph
  merges state fields). It keeps the test off the brittle
  Phase-1-3-recon-discovers-2-GoAD-hosts path (the Lame recon plan
  fixture targets 10.10.10.5 — re-authoring it for two GoAD hosts
  would explode the fixture set).
"""
...
async def test_phase5_full_pipeline(
    ...
    """Mocked Phase 5 pipeline: recon → vuln → exploit → postex → lateral → cleanup → report.

    Verifies the full Phase 5 graph runs end-to-end with all LLM +
    subprocess + subagent + EventBus boundaries mocked, and that the
    final state is populated with every Phase 5 field (pivots,
    movement_paths, sub_engagements, cleanup_results) PLUS the Phase
    1-4 carry-overs (hosts, footholds, local_users, harvested_secrets,
    persistence_artifacts, phase="done").
    """
    ...
    # assertion-wise). Matches Phase 3 / 4 T14.
    ...
    def _fake_sub_state():
        sub = EngagementState(
            engagement_id="p5-pipeline_sub_01",
    ...
    with ExitStack() as stack:
        ...
        # --- Build and run Phase 5 graph -------------------------------
        # checkpointer=None keeps the test off the persistence layer
        # (asyncio.Queue handles inside the bus aren't msgpack-
        # serialisable).
        graph = build_phase5_graph(checkpointer=None)
        config = {
            "configurable": {
    ...
    # --- Verify final state ---------------------------------------------
    # Report stub overwrites cleanup's phase="report" with "done".
    ...
    assert movement_paths[0].from_host == "192.168.56.22", (
        f"Expected movement_paths[0].from_host=192.168.56.22; got "
        f"{movement_paths[0].from_host!r}"
    )
    assert movement_paths[0].to_host == "192.168.56.11", (
        f"Expected movement_paths[0].to_host=192.168.56.11; got "
        f"{movement_paths[0].to_host!r}"
    )
    ...
    assert ref.sub_id == "p5-pipeline_sub_01", (
        f"Expected sub_id=p5-pipeline_sub_01; got {ref.sub_id!r}"
    )
    assert ref.status == "completed", (
        f"Expected sub-engagement status=completed; got {ref.status!r}"
    )
    assert ref.sub_state_path == (
        "engagements/p5-pipeline_sub_01/state.json"
    ), f"Unexpected sub_state_path: {ref.sub_state_path!r}"

    # 4. Cleanup removed + verified every artifact (spec §13.5).
    # At minimum: 1 removal + 1 verification for the pre-seeded a-1
    # artifact on 192.168.56.22, plus 1 temp-file unlink for the
    # staged linpeas_upload.sh. All verified (the mock payloads return
    # verified=True for both removal and verification results — the
    # sub-agent isolation tests in Tasks 8/9 cover the verified=False
    # failure path).
